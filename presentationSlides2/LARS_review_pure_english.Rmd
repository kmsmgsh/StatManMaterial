...
#### So the problem becomes to another one, why the form of vector making equal angles? 
...
还是老样子从 $\hat\mu_0=0$ 开始，所以还是两件事，计算当前的correlation  $\hat c=X'(y-\hat\mu_{\mathcal{A}})$ . 其中 $\mathcal{A}$ 是一个indices set包括了每一步最大相关系数。这里有点奇怪。特别是 $\hat C$ 的定义形式。
  ...
  ???
  但是 $\mathcal{A}$  这样定义感觉很奇怪啊。 $\mathcal{A}$是 投影向量的长度。 $\mathcal{A}$里面只包含了correlation是最大的那几个，也就是说一直走一直走，走到有新的correlation能加进来，那再改变方向，重新计算 $u_{\mathcal{A}}$，否则就一直按这个方向走。比如说回到figure2的图例，在一开始，最大的correlation只有 $x_1$这个方向，走一小步， $x_2$ 方向的correlation还是小于 $x_1$ ，所以 $\mathcal{A}$还是只有{1}。直到residual 的correlation到 $x_1$和 $x_2$ 一致，那么就有了两个同时达到max的correlation，那就得重新计算 $u_\mathcal{A}$ .  
...
Because the construction is under the property of "equal angle", so the correlation between residuals with covariate inside $\mathcal{A}$ is change simultaneously with same value.

$$X _ { k } ^ { \prime } \left( \mathbf { y } - \widehat { \boldsymbol { \mu } } _ { k - 1 } \right) = \widehat { C } _ { k } 1 _ { \mathcal { A } }$$
Since $u_k$  is a unit vector, (29) ， $\overline y_k$ then we have $\overline { \mathbf { y } } _ { k } - \widehat { \boldsymbol { \mu } } _ { k - 1 }$ with length
$$\overline { \gamma } _ { k } \equiv \frac { \widehat { C } _ { k } } { A _ { k } }$$



???
和update的那个公式进行比较，则LARS的估计在 the vector $\hat\mu_k$ 在 $\hat\mu_{k-1}$到 $\overline y_k$ 的延长线上.
---
Comparing this with the formula for updating, the estimate of LARS $\hat\mu_k$  is in the extend from the $\hat\mu_{k-1}$ to $\overline y_k$.
...
A problem rised here, $\hat\gamma_k$ is always lower than $\overline \gamma_k$，so LARS estimates always approaching but never reaching the OLS estimates $\overline y_k$.

Except an extreme situation, if LARS include all the covariates, then it is equivelant to OLS.
Because LARS can "one-step-reach", it is efficient in computation.
...
Lasso solutions can be generated by a minor modification of LARS algorithm. It is related to homotopy method，though the LARS approach is somewhat more direct.
...
So the sign of $\hat\beta_j$, should consistent to the sign of current covariate with residuals. That is, the direction of move should equivelent to the correlation, cannot move in a negative way.
...
(This definition is equivelent to the original LARS)
...
If $\tilde \gamma$ is less than $\hat\gamma$. That is, the sign changed in the half of moving the solution path.
